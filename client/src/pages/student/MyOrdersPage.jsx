          ...
          <div className="flex overflow-x-auto w-full md:w-auto no-scrollbar space-x-2 p-1">
            {tabs.map(tab => (
              <button
                ...
                className={`px-6 py-2.5 rounded-xl text-sm font-bold whitespace-nowrap transition-colors cursor-pointer ${activeTab === tab
        ...
        <div className="space-y-4">
          {filteredOrders.length === 0 ? (
            ...
            filteredOrders.map(order => (
              <div
                key={order.id}
                className={`bg-white rounded-3xl p-5 border shadow-sm transition-all flex flex-col sm:flex-row gap-6 ${order.status === 'preparing'
