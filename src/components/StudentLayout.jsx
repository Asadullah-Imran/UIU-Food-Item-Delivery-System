  ...
  return (
    <div className="flex h-screen bg-[#F9FAFB] font-sans text-slate-800 overflow-hidden relative">
      ...
      <div className={`fixed inset-y-0 left-0 transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} lg:relative lg:translate-x-0 w-64 bg-white border-r border-slate-200 flex flex-col h-full flex-shrink-0 z-30 transition-transform duration-300 ease-in-out shadow-2xl lg:shadow-none`}>
        ...
        <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
          ...
          <Link 
            ...
            onClick={() => setIsMobileMenuOpen(false)}
            className={`flex items-center px-4 py-3 rounded-xl font-semibold transition-all ${currentPath === '/dashboard/student/orders' ? 'bg-orange-500 text-white shadow-md shadow-orange-500/20' : 'text-slate-600 hover:bg-slate-50 hover:text-orange-500'}`}
          >
          ...
          <Link 
            ...
            onClick={() => setIsMobileMenuOpen(false)}
            className={`flex items-center px-4 py-3 rounded-xl font-semibold transition-all ${currentPath === '/dashboard/student/chat' ? 'bg-orange-500 text-white shadow-md shadow-orange-500/20' : 'text-slate-600 hover:bg-slate-50 hover:text-orange-500'}`}
          >
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative z-10 w-full">
        ...
        <main className={`flex-1 overflow-y-auto bg-[#F9FAFB] ${noPadding ? 'p-0' : 'p-4 sm:p-8'}`}>
          {children}
        </main>
      </div>
    </div>
